...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    // Prismatic effect with sharp color bands and geometric patterns
    number angle = atan(uv.y - 0.5, uv.x - 0.5);
    number distance_from_center = length(uv - vec2(0.5, 0.5));
    number prismatic_shift = mod(angle + time * 0.8 + distance_from_center * 3.0, 6.28318);
    
    // Create sharp rainbow bands
    tex.r = tex.r*0.8 + (0.5 + 0.5*sin(prismatic_shift)) * maxfac * 0.8 + (0.1*prismatic.y);
    tex.g = tex.g*0.75 + (0.5 + 0.5*sin(prismatic_shift + 2.094)) * maxfac * 0.8;
    tex.b = tex.b*1.3 + (0.5 + 0.5*sin(prismatic_shift + 4.189)) * maxfac * 0.8 + (0.000001*prismatic.x);

    // Add extra brightness for prismatic effect
    tex.rgb += vec3(0.2) * maxfac * (0.7 - fac5 * 0.27);
